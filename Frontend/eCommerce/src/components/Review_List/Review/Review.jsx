...
function Review({reviewData}) {
  const [displayFullBody, setDisplayFullBody] = useState(false);

  function showFullBody() {
    setDisplayFullBody(true);
  }

  function bodySizing() {
    if (reviewData.body.length < 150 || displayFullBody == false) {
    ...
    return <p className={classes.review__body}>{reviewData.body}</p>
  }
